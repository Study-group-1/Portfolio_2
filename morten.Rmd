...
sum_m4_sq <- summary(m4_sq)
residuals <- sum_m4_sq$residuals

#making a df with actual values and predicted values
exciting <- data.frame("actual" = df$CHI_MLU, "residuals" = sum_m4_sq$residuals, "predicted" =df$CHI_MLU + sum_m4_sq$residuals)
...
mot2 <- lmerTest::lmer(MOT_MLU ~ VISIT * Diagnosis + (1|SUBJ), df, REML = F)
...
m <- lmer(CHI_MLU ~ VISIT * Diagnosis * Age * Gender + MOT_MLU + Gender + Ethnicity + MullenRaw + ExpressiveLangRaw + Socialization + (1|SUBJ), df, REML = F)

m_10 <- update(m, .~. + I(Diagnosis)^10 + I(VISIT)^10 )

MuMIn::r.squaredGLMM(m_10)
