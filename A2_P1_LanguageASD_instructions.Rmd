...
author: "Magnus, Tobias, Gustav, Morten, Nicoline"
date: "10.09.20"
...
=======
pacman::p_load(ggplot2, tidyverse, MuMIn, lme4, purrr, lmerTest)

<<<<<<< HEAD
pacman::p_load('tidyverse', 'ggplot2', 'lme4', 'lmerTest', 'MuMIn', 'pastecs')

=======
>>>>>>> f6a46a2416e96bc4307bd29af770a1d91b2b38de
>>>>>>> 8b94832978b643c4cc3624bd27d675f63c71212d
```
...
<<<<<<< HEAD
df <- read_csv("autisme")
#Removing column X1 from Autism dataset
df$X1 <- NULL
=======
df <- read_csv('autisme')
df$X1 <- NULL
...
df %>% split(df$Diagnosis) %>% map(summary)
...
df %>% 
  ...
  split(df$Diagnosis) %>% map(summary)

```

The sample included mostly young (<20) white males ...

- Approximately the same amount of children 
- Not equally 
- ASD matched with TD at first visit -- we're interested in the development of the children. 
- A very large skew in Gender towards males 
...
m1 <- lmerTest::lmer(CHI_MLU ~ Age + Diagnosis + ( 1  |SUBJ), df, REML = F)
...
df %>%
  ggplot(aes(x = Age, y = CHI_MLU, color = Diagnosis)) + 
...
```{r ex3, include = FALSE}
m2 <- lmerTest::lmer(MOT_MLU ~ Age + Diagnosis + ( 1  |SUBJ), df, REML = F)
